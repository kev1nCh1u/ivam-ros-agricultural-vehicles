class onewheel_vw: public Move_Robot
{
    public:
      ...
      //EV PedalCallback
  		void EV_PedalCallback(const move_robot::joystick &Pedal_msg);
      ...
      //Farm AGV Vision Navigation
      void Vision_Navi();

      //Farm AGV Magnetic Navigation
      void Magnetic_Navi();
      ...
      //EV Pedal Throttle and Brake
      float Pedal_Throttle_Value;
      float Pedal_Brake_Value;
...
onewheel_vw::onewheel_vw(char *dev_name, int Baudrate):Move_Robot(dev_name, Baudrate)
{
  ...
  Pedal_Throttle_Value = 0.0;
  Pedal_Brake_Value = 0.0;
  ...
  //EV Pedal Subscriber
  EV_Pedal_Subscriber_ = node_.subscribe("EV_Pedal", 5, &onewheel_vw::EV_PedalCallback, this);
...
void onewheel_vw::Update_EV_Pose()
{
...
void onewheel_vw::Magnetic_Navi()
{
  ...
  else if(!Magnetic_Event_Trigger)
  {
...
void onewheel_vw::EV_PedalCallback(const move_robot::joystick &Pedal_msg)
{
	//std::cout << "=============================" << std::endl;
	//std::cout << "=============================test" << std::endl;
	Pedal_Throttle_Value = Pedal_msg.x;
  Pedal_Brake_Value = Pedal_msg.y;
  // std::cout << "Pedal_Throttle_Value: " <<Pedal_Throttle_Value<< std::endl;
  // std::cout << "Pedal_Brake_Value: " <<Pedal_Brake_Value<< std::endl;
	//std::cout << "=============================" << std::endl;
}
...
// 			drawLine(5, 0.0 ,0.0 ,1.0,test);
			...
			Car.bicycle_kinematics(V_rv,W_rw,V_rev,delta);
    ...
    //std::cout<<"AA"<<std::endl;
			...
			if(kinematics_model == "diff")
      {
  				...
  				if(fabs(angular_velocity_error) > M_PI)
          {
  				...
  				if(fabs(cmd_angular_velocity) >= 0.35)
          {
    					if(cmd_angular_velocity > 0) cmd_angular_velocity = 0.35;
    					else cmd_angular_velocity = -1*0.35;
  				}
  				//w過大會減速（意味可能有再轉彎）
  				if(v_buf > Min_nav_speed )
          {
			...
			if(kinematics_model == "diff")
      {
		...
		else if(type == MISSON_ChangeMode)
		{
			if( dis_error <= ChangeMode_dis_error){
				// rpm[0] = 0;
				// rpm[1] = 0;
				// rpm[2] = 0;
				// rpm[3] = 0;
		...
		else if( dis_error <= othermode_dis_error){
			// rpm[0] = 0;
			// rpm[1] = 0;
			// rpm[2] = 0;
			// rpm[3] = 0;
...
std::cout<<"see 5  "<<std::endl;
	...
	void onewheel_vw::joystick_move()   // Important: jeff said this function is running by timer!!!!!!
	{//==============change=============
		//std::cout<<"btn_id  "<<btn_id<<std::endl;
      ...
      if(Send_EV_Steering >= 18)Send_EV_Steering = 18;
      else if(Send_EV_Steering <= -18)Send_EV_Steering = -18;
			//==============change=============

			float V_avg_throttle = Pedal_Throttle_Value;
      float V_avg_brake = Pedal_Brake_Value;
      float V_avg = joystick_v;
			if(V_avg_throttle < 0.07)
				V_avg_throttle = 0.0;
      if(V_avg_brake < 0.07)
				V_avg_brake = 0.0;
      if(V_avg*cos(joystick_theta)<0)
      {
        ...
        float Send_EV_Velocity_Neg = -V_avg_brake * (velocity_max - velocity_min) + velocity_min;
          ...
          //Send_EV_Steering = -Send_EV_Steering;
    			...
    			SendPackage(command);
    			std::cout<<"Send_EV_Velocity_Neg: "<< Send_EV_Velocity_Neg << "Send_EV_Steering: " << Send_EV_Steering <<std::endl;
    		}
		}
