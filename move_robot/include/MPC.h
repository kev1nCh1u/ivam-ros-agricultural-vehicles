...
//8000 0.06 11 0.05 30
    const int cte_cost_weight = 6500;//8000
    const int epsi_cost_weight = 500;//2500
    const int v_cost_weight = 1000;
    const int delta_cost_weight = 100;//45
    const int a_cost_weight = 1;
    const int delta_change_cost_weight = 20;
    const int a_change_cost_weight = 1;
...
fg[1 + x_start + t] = x1 - (x0 + v0 * CppAD::cos(psi0) * dt);
fg[1 + y_start + t] = y1 - (y0 + v0 * CppAD::sin(psi0) * dt);
fg[1 + psi_start + t] = psi1 - (psi0 + (v0 * CppAD::tan(delta0 * M_PI / 180) * dt) / 2.52);
fg[1 + v_start + t] = v1 - (v0 + a0 * dt);
fg[1 + cte_start + t] = cte1 - ((f0-y0) + (v0 * CppAD::sin(epsi0) * dt));
fg[1 + epsi_start + t] = epsi1 - ((psi0 + psi_des0) + (v0 * CppAD::tan(delta0 * M_PI / 180) * dt) / 2.52);
...
//   std::cout<<"epsi "<< epsi <<std::endl;
  ...
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.25; // for IVAM EV ----> maximum auto speed is 1.4m/s and maximum steering angle limit is 25 degree
    vars_upperbound[i] = 0.25; // for IVAM EV ----> maximum auto speed is 1.4m/s and maximum steering angle limit is 25 degree
  }
