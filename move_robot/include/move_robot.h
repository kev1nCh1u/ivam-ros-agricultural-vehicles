...
public:
	...
	//EV Pose Callback
	void EV_Pose_Callback(const geometry_msgs::PoseStamped& EV_Pose_msg);
...
//	ros::Subscriber Clear_ObsModeSubscriber_;
	...
	//EV Pose Subsciber Declearation
	ros::Subscriber EV_Pose_Subscriber_;
	//EV Pose Publisher to RVIZ
	ros::Publisher EV_Pose_Publisher_;

	//Farm AGV Vision Pose Subscriber
	ros::Subscriber Farm_AGV_Vision_Pose_Subscriber_;
	ros::Subscriber Farm_AGV_Magnetic_Offset_Subscriber_;
	ros::Subscriber Farm_AGV_RFID_Subscriber_;

	//EV Pedal Subsciber Declearation
	ros::Subscriber EV_Pedal_Subscriber_;
  ...
  tf::MessageFilter<geometry_msgs::PoseWithCovarianceStamped>* initial_pose_filter_;
	...
	//EV GPS & IMU Pose
	Eigen::Vector3f EV_Pose_;
	Eigen::Vector3f EV_Pose_Vec;
	Eigen::Vector3f EV_Pose_Initial;
  ...
  std::vector<int> theta;
	...
	std::string Farm_AGV_RFID_Event;
...
Move_Robot::Move_Robot(char *dev_name, int Baudrate)
{
    ...
    //EV Subsciber
    EV_Pose_Subscriber_ = node_.subscribe("Send_Pose", 5, &Move_Robot::EV_Pose_Callback, this);
    //EV Publisher
    EV_Pose_Publisher_ = node_.advertise<geometry_msgs::PoseStamped>("Draw_EV_Pose", 1000);
		...
		Farm_AGV_RFID_Subscriber_ = node_.subscribe("RFID_Command", 5, &Move_Robot::Farm_AGV_RFID_Callback, this);
...
void Move_Robot::CreatPath(std::vector<Eigen::Vector3f> input_buf,std::vector<int> &id_Point_lineMode,std::vector<float> radius, SUB_MISSONPATH_SUBPOINT &return_buf)
{
    ...
    for(int i=0; i<input_buf.size()-1; i=count_index){
        ...
        std::cout<<"input_buf[0] "<<input_buf[0]<<std::endl;
        std::cout<<"input_buf[1] "<<input_buf[1]<<std::endl;
        std::cout<<"input_buf[2] "<<input_buf[2]<<std::endl;
        ...
        case 1://Arc
            ...
            // std::cout<<"Cornervector "<<Cornervector <<std::endl;
...
void Move_Robot::EV_Pose_Callback(const geometry_msgs::PoseStamped& EV_Pose_msg)
{
    //tf::Pose EV_pose_tf;
    //tf::poseMsgToTF(EV_Pose_msg.pose, EV_pose_tf);
    EV_Pose_ = Eigen::Vector3f(EV_Pose_msg.pose.position.x, EV_Pose_msg.pose.position.y, EV_Pose_msg.pose.position.z);
    //EV_Pose_ = Eigen::Vector3f(-40.9815, -51.745,0.0);
    Fix_IMU(EV_Pose_msg,EV_Pose_Vec);
    slam_pose_ = EV_Pose_Vec;
    //std::cout<<"EV_Pose_======"<<EV_Pose_<<std::endl;
    //ROS_INFO("Robot pose x: %f y: %f yaw: %f", slam_pose_[0], slam_pose_[1], slam_pose_[2]);
...
void Move_Robot::Farm_AGV_RFID_Callback(const std_msgs::String &RFID_msg)
{
	...
	else if(Farm_AGV_RFID_Event == "bye")
	{
    ...
    if(!fin.is_open())
    {
        ...
        if(s_Car_packMode == "ChinaMotor")
            ...
            Car_packMode = 2;
		        ...
		        Car_packMode = 0;
		    	else if(s_Car_packMode =="IVAM_EV")
		    	Car_packMode = 1;
...
void Move_Robot::initialPoseCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg)
{
		tf::Pose pose;
		...
		Eigen::AngleAxisd rot(initial_pose_.z(),Eigen::Vector3d(0,0,1));
		...
		a.pose.position.x = initial_pose_.x();
		a.pose.position.y = initial_pose_.y();
...
{
	EV_Path_Subpoint.start = 0;
	EV_Path_Subpoint.end = 10;
	EV_Path_Subpoint.start_type = 0;
	EV_Path_Subpoint.end_type = 3;
	...
	std::fstream fin;
	    ...
	    else
	    {
	    	...
	    	{//std::cout<<"x   "<<i<<std::endl;
	    		...
	    		//test.push_back(p);
	    		EV_Path_Subpoint.sub_missonPath_subPoint.push_back(p);
	    	}
    ...
    draw(TrackingLineMarker , 1.0, 0.0, 0.0, EV_Path_Subpoint.sub_missonPath_subPoint);
}

void Move_Robot::LoadPath()
{
	//Fake Data for ST Node_recv
	NODE_recv ST ;
	ST.id = 0;
	ST.type = 0;
	ST.kin = "diff";
	...
	ST.line = 1;
	ST.radius = 5;
	ST.node_pose = Eigen::Vector3f(10.0,20.0,30.0);

	//Fake Data for EN Node_recv
	NODE_recv EN ;
	EN.id = 10;
	EN.type = 3;
	EN.kin = "diff";
	...
	EN.line = 1;
	EN.radius = 5;
	EN.node_pose = Eigen::Vector3f(10.0,20.0,30.0);


    if(Check_EV_Pose == true)
    {

	    SUB_MISSONPATH EV_Path_Point;
	    EV_Path_Point.ALL_pathnode.push_back(ST);
	    EV_Path_Point.ALL_pathnode.push_back(EN);
	    EV_Path_Point.sub_missonPath.push_back(EV_Path_Subpoint);




	    A_misson.push_back(EV_Path_Point);
	    p_state_ = P_STATE_MOVE;
	    Check_EV_Pose = false;
    }
    ...
    {
    	ROS_INFO("Error: EV Pose haven't been set!! Please Check IMU Heading in Rviz");
    }
}
