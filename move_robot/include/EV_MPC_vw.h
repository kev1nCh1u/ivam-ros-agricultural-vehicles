...
class EV_MPC_vw: public Move_Robot
{
    public:
      EV_MPC_vw(char *dev_name, int Baudrate);
      ~EV_MPC_vw();
...
EV_MPC_vw::EV_MPC_vw(char *dev_name, int Baudrate):Move_Robot(dev_name, Baudrate)
{
  //MPC mpc;
  ...
  Clear_ObsModeSubscriber_=node_.subscribe("Clear_ObsMode", 10, &EV_MPC_vw::ClearCallback,this);
  //laserSubscriber_ = node_.subscribe("scan", 10, &EV_MPC_vw::laserCallback, this);
  JoysickSubscriber_ = node_.subscribe("joystick", 5, &EV_MPC_vw::joystickCallback, this);
  TriggerSubscriber_=node_.subscribe("Trigger", 5, &EV_MPC_vw::TriggerCallback_all, this);
  ...
  initial_pose_filter_->registerCallback(boost::bind(&EV_MPC_vw::initialPoseCallback, this, _1));



  receive_Battery_thread_ = new boost::thread(boost::bind(&EV_MPC_vw::RevProcess_Battery, this, 0.1));
  receive_thread_ = new boost::thread(boost::bind(&EV_MPC_vw::RevProcess, this, 0.01));
  control_timer = node_.createTimer(ros::Duration(time_sample), &EV_MPC_vw::timerCallback, this);

  //std::cout<<"satParameter_two "<<std::endl;
  mpc.satParameter_two(8,0.3,LWheeldis);
  //std::cout<<"satParameter_two "<<std::endl;
...
void EV_MPC_vw::TriggerCallback_all(const std_msgs::Int8& msg)
{
...
void EV_MPC_vw::timerCallback(const ros::TimerEvent& event)
{
...
void EV_MPC_vw::Update_EV_Pose()
{
	//std::cout<<"slam_pose_  "<<slam_pose_<<std::endl;
	Eigen::AngleAxisd rot(slam_pose_.z(),Eigen::Vector3d(0,0,1));
	...
	a.pose.position.x = slam_pose_.x();
	a.pose.position.y = slam_pose_.y();
	a.pose.orientation.w = Qu.coeffs()[3];
	a.pose.orientation.z = Qu.coeffs()[2];
	EV_Pose_Publisher_.publish(a);
}

void EV_MPC_vw::ClearCallback(const std_msgs::Int8& msg)
{
...
void EV_MPC_vw::State_Machine(int& state)
{
int type,id;
	switch (state) {
...
bool EV_MPC_vw::Navigation_move(std::vector<SUB_MISSONPATH_SUBPOINT> & myAllSubPath, bool isReSet)
{
...
bool EV_MPC_vw::Trajectory_Tracking(int &subpath_index, bool isReSet)//isReSet是fault動作 //true是不動做
{
...
bool EV_MPC_vw::Tracking_Angle_Init(int &subpath_index, bool isReSet)
{
  return true;
...
// 			drawLine(5, 0.0 ,0.0 ,1.0,test);
			...
			Car.mpc_bicycle_kinematics(V_rv,W_rw,V_rev,delta);
...
bool EV_MPC_vw::Tracking_Trajectory(int &subpath_index, bool isReSet)
{
  ...
  //////////////////////////////////////////////////////////
	...
	//MPC
	double v_recv = V_cal;//要回受速度給這個
	double delta_recv = Delta_now;//要回受速度給這個
	double a = a_cal;//要回受速度給這個
	...
	if(!isReSet){
		...
		//slam_pose_ = Eigen::Vector3f(-40.6815,-51.245,0.0);
		...
		//////////////////////////
		//查找目前所應追尋的點
		target_ind = calc_target_index(robot_pos, V_cal, A_misson[ready_path_index].sub_missonPath[subpath_index].sub_missonPath_subPoint, now_index);
		//目標點
		target_pos = A_misson[ready_path_index].sub_missonPath[subpath_index].sub_missonPath_subPoint[target_ind];
		...
		//隨時監控使否要轉彎以縮小閉障範圍
		int obs_turn_int = calc_target_index_obsturn(robot_pos, V_cal, A_misson[ready_path_index].sub_missonPath[subpath_index].sub_missonPath_subPoint, now_index);
		...
		if(world_localreal_path.size() < 10)
		{
			for(int i = 0; i <= 20; i+=2)
			{
				Eigen::Vector3f p;
				p = MPC_add_Path[now_index + i];
				world_localreal_path.push_back(p);
			}
		...
		double cte = 0.0;
		double epsi = 0.0;
		if(!Endangle && world_localreal_path.size() > 10)
		{
			std::cout<<" >5 "<<std::endl;

			draw(6, 0.0, 1.0, 0.0,world_localreal_path);
		...
		double pred_px = 0.0 + V_cal * dt; // Since psi is zero, cos(0) = 1, can leave out
		double pred_py = 0.0; // Since sin(0) = 0, y stays as 0 (y + v * 0 * dt)
		double pred_psi = local_heading +  (V_cal * tan((float)Delta_now * M_PI/180.0 )) * dt / 2.52;
		double pred_v = V_cal + a * dt;
		double pred_cte = cte + V_cal * sin(epsi) * dt;
		double pred_epsi = epsi + local_heading  +  (V_cal * tan((float)Delta_now* M_PI/180.0)) * dt / 2.52;
...
std::cout<<"Endangle "<<Endangle  <<std::endl;
	if(!Endangle){
		std::cout<<"now_index "<<now_index  <<std::endl;
		std::cout<<"First_tracking "<<First_tracking  <<std::endl;
		//===前85% 的導航====
		if(now_index < First_tracking ){
			...
			//std::cout<<"local_heading "<<local_heading  <<std::endl;
			//std::cout<<"cte "<<cte  <<std::endl;
			//std::cout<<"epsi "<<epsi  <<std::endl;
			...
			// if(fabs(V_sub_target) <= 1.0){
			// 		if(V_sub_target > 0) V_sub_target =1.0;
			// 		else V_sub_target = -1*1.0;
			// }
...
void EV_MPC_vw::ObsDetect(std::vector<Eigen::Vector3f> &i_obs_container,
			std::vector<Eigen::Vector3f> &i_obs_region_0,
			std::vector<Eigen::Vector3f> &i_obs_region_1,
			std::vector<Eigen::Vector3f> &i_obs_region_2,
			std::vector<Eigen::Vector3f> &i_obs_region_3,
	...
	void EV_MPC_vw::ObsStateMachine(std::vector<Eigen::Vector3f> &i_obs_region_0,
			std::vector<Eigen::Vector3f> &i_obs_region_1,
			std::vector<Eigen::Vector3f> &i_obs_region_2,
			std::vector<Eigen::Vector3f> &i_obs_region_3,
			std::vector<Eigen::Vector3f> &i_obs_region_4,
	...
	void EV_MPC_vw::RePlanning(int &subpath_index,
			std::vector<Eigen::Vector3f> &i_obs_gap_0,
			std::vector<Eigen::Vector3f> &i_obs_gap_2,
			std::vector<Eigen::Vector3f> &i_obs_gap_4,
			std::vector<Eigen::Vector3f> &all_point)
	...
	bool EV_MPC_vw::FindKeyPoint(Eigen::Vector3f& target, Eigen::Vector3f& key_p,
			std::vector<Eigen::Vector3f> &i_obs_gap_0,
			std::vector<Eigen::Vector3f> &i_obs_gap_2,
			std::vector<Eigen::Vector3f> &i_obs_gap_4,
			std::vector<Eigen::Vector3f> &all_point)
...
std::cout<<"see 5  "<<std::endl;
	...
	void EV_MPC_vw::CreateNewPath(Eigen::Vector3f& target, Eigen::Vector3f& key_point, int subpath_index, std::vector<Eigen::Vector3f> &new_path)
	{
	...
	void EV_MPC_vw::joystickCallback(const move_robot::joystick& joystick)
	{
		//std::cout<<"btn_id  "<<btn_id<<std::endl;


		if(btn_id != PUSE_BUTTON_Y && btn_id != PUSE_BUTTON_BACK)
		{
			btn_id = joystick.btn_id;
		}


		//float vx = joystick.x * JOYSTICK_SCALAR;   //for AGV
		//float vy = joystick.y * JOYSTICK_SCALAR;   //for AGV

		float vx = joystick.x;   //for EV
		float vy = joystick.y;   //for EV


		//joystick_v = EV_velocity;   //for EV
		...
		//std::cout<<"EV_velocity: "<< EV_velocity << "EV_steering: " << EV_steering <<std::endl;
		//std::cout<<"vx: "<<vx<<" vy: "<<vy <<std::endl;

	}
	void EV_MPC_vw::joystick_move()   // Important: jeff said this function is running by timer!!!!!!
	{//==============change=============
		//std::cout<<"btn_id  "<<btn_id<<std::endl;
		...
		{
			std::vector<unsigned char> command;
			//float Send_EV_Steering = joystick_theta;   //for EV
			//float Send_EV_Velocity = joystick_v;   //for EV
			...
			float Send_EV_Steering = W_rw * 100;
      Send_EV_Steering = -Send_EV_Steering;
			if(Send_EV_Steering >= 20)Send_EV_Steering = 20;
			else if(Send_EV_Steering <= -20)Send_EV_Steering = -20;
			//==============change=============
			...
			if(V_avg*cos(joystick_theta)<0)
			{
      		...
      		float Send_EV_Velocity = V_avg * 600;
      		if(Send_EV_Velocity >= 0)
      		{
      			std::vector<unsigned char> command;
      		...
      		else if(Send_EV_Velocity < 0)
      		{
      			std::vector<unsigned char> command;
      			float Send_EV_Velocity_Neg = -Send_EV_Velocity;
            Send_EV_Steering = -Send_EV_Steering;
      			if(Send_EV_Velocity_Neg >= 600)
      			{
      				Send_EV_Velocity_Neg = 600;
      			}
      			sendreceive.Package_OneWheel_encoder(0, Send_EV_Steering,1,1,1, command);
      			SendPackage(command);
      			std::cout<<"Send_EV_Velocity_Neg: "<< 0 << "Send_EV_Steering: " << Send_EV_Steering <<std::endl;
      		}
		}
		else if(btn_id == PUSE_BUTTON_X)   //EV manual mode D
		{
			std::vector<unsigned char> command;
			sendreceive.Package_OneWheel_encoder(0,0,0,2,0,command);
			SendPackage(command);
			std::cout<<"Change to manual mode: D "<<std::endl;
		}
		else if(btn_id == PUSE_BUTTON_B)   //EV manual mode R
		{
			std::vector<unsigned char> command;
			sendreceive.Package_OneWheel_encoder(0,0,0,0,0,command);
			SendPackage(command);
			std::cout<<"Change to manual mode: R "<<std::endl;
		}
		else if(btn_id == PUSE_BUTTON_Y)
		{
			LoadPath();
			std::cout<<"Create Navigation Path"<<std::endl;
	...
	void EV_MPC_vw::RevProcess(double receive_period)
	{
		...
		while(1){
			...
			if(mySerial.serial_ok == true){
				...
				if(rev_buf.size() == 8)
				{
					sendreceive.RevProcess_IVAM_EV_encoder(rev_buf, RPM_L, RPM_R, Delta_now, V_cal, a_cal);
					rev_buf.clear();
				}
	...
	void EV_MPC_vw::RevProcess_Battery(double receive_period)
{
		int Receive_Package_Size = 256;
	...
	void EV_MPC_vw::Caculate_W_rw(float stop_angle, Eigen::Vector3f robot_pos, float &angular_error, float &pre_angular_error, float &cmd_angular_velocity, bool special)
	{
