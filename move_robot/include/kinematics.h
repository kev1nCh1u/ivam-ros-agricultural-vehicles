...
public:
    ...
    void bicycle_kinematics(float V, float omega, float &V_rev, float &delta);
    void mpc_bicycle_kinematics(float V, float delta_rad, float &V_rev, float &delta);
...
void Car_Kinematics::bicycle_kinematics(float V, float omega, float &V_rev, float &delta)
{
    float L = 2.52; //EV Axle Length (meters)
    

    V_rev = (V / ( M_PI * 0.55)) * 60.0 * 10.0; //from m/s to RPM. 10  ---> Reduction Ratio
    delta = atan((omega * L) / V)*180.0/M_PI;
    ...
    std::cout<<"bicycle_kinematics========  " <<std::endl;
    std::cout<<"V_rev  "<<V_rev <<std::endl;
    std::cout<<"delta  "<<delta <<std::endl;

}

void Car_Kinematics::mpc_bicycle_kinematics(float V, float delta_rad, float &V_rev, float &delta)
{
    float L = 2.52; //EV Axle Length (meters)
    

    V_rev = (V / ( M_PI * 0.55)) * 60.0 * 10.0; //from m/s to RPM. 10  ---> Reduction Ratio
    delta = delta_rad * 180.0 / M_PI;
