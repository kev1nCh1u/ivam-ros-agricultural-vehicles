...
Control::Control(char *address, char *SRV_IP, int port)
{
  ...
  SetLocalpar(LocalparPATH);
	...
	//Farm AGV Pose Message
	Farm_AGV_Vision_Pose = "";
	Farm_AGV_Magnetic_Offset = 0.0;
	...
	if(isSocketOpen_){
		...
		//Farm AGV Vision Publish
		Farm_AGV_Vision_Publisher_ = nodeHandle_.advertise<std_msgs::String>("Vision_Command",10);
		Farm_AGV_Magnetic_Publisher_ = nodeHandle_.advertise<std_msgs::Float32>("Magnetic_Command",10);
		Farm_AGV_RFID_Publisher_ = nodeHandle_.advertise<std_msgs::String>("RFID_Command",10);
...
void Control::RevProcess(double receive_period)
{
	...
	//for Farm AGV
	bool isFarmAGV_Vision = false;
	bool isFarmAGV_Magnetic = false;
	bool isFarmAGV_RFID = false;
		...
		if(n == -1){
			...
			while(!recv_pkg[count].empty())
			{
				...
				while(getline(cut,cut_par,','))
				{
					...
					else if(recv_pkg_subitem[0]=="Z"&&count==0) //（ﾟДﾟ）σ弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌⊃ Vision
					{
						count1++;
						isFarmAGV_Vision = true;
					}
					else if(recv_pkg_subitem[0]=="Y"&&count==0) //（ﾟДﾟ）σ弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌⊃ Magnetic
					{
						count1++;
						isFarmAGV_Magnetic = true;
					}
					else if(recv_pkg_subitem[0]=="R"&&count==0) //（ﾟДﾟ）σ弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌弌⊃ RFID
					{
						count1++;
						isFarmAGV_RFID = true;
					}
					...
					if(isFarmAGV_Vision)
					{
						...
						{
							Farm_AGV_Vision_Pose = "stop";
							break;
						}
					}

					//==============Farm AGV Magnetic Navigation==============
					if(isFarmAGV_Magnetic)
					{
						if(count>0&& count<end)
						{
							std::istringstream item(recv_pkg_subitem[count1]);
					...
					//=============Farm AGV Magnetic Navigation (RFID Tag)==============
					if(isFarmAGV_RFID)
					{
						if(count>0&& count<end)
						{
			...
			if(isFarmAGV_Vision)
			{
				std_msgs::String Vision_msg;

				Vision_msg.data = Farm_AGV_Vision_Pose;

				Farm_AGV_Vision_Publisher_.publish(Vision_msg);
				Farm_AGV_Vision_Pose = "";
			}

			if(isFarmAGV_Magnetic)
			{
				std_msgs::Float32 Magnetic_msg;

				Magnetic_msg.data = Farm_AGV_Magnetic_Offset;

				Farm_AGV_Magnetic_Publisher_.publish(Magnetic_msg);
				Farm_AGV_Magnetic_Offset = 0.0;
			}
			...
			if(isFarmAGV_RFID)
			{
				...
				RFID_msg.data = Farm_AGV_RFID_Tag;

				Farm_AGV_RFID_Publisher_.publish(RFID_msg);
				Farm_AGV_RFID_Tag ="";
			}
...
void Control::slamoutCallback(const geometry_msgs::PoseStamped& msg)
{
	...
	if(count==20)
	{
		...
		std::string DataFlow = head + CarName + "," + Carnumber + "," + IP_buf + "," + MAP_NAME + ","
	...
	if(SendNode)
	{
		...
		std::string DataFlow = "N;"  + MAP_NAME + "," +robot_pose_x + "," + robot_pose_y + "," + robot_pose_z + "," + real_pose_x + "," + real_pose_y + "," + real_pose_z  + tail ;
...
void Control::sendScale()
{
	...
	std::string DataFlow = head + CarName + "," + Carnumber + "," + IP + "," + MAP_NAME + "," + scale +  tail ;
