...
Joystick::Joystick(char *joystick_dev)
{
	...
	EV_Pedal_Publisher_ = nodeHandle_.advertise<JoyStick::joystick>("EV_Pedal", 10);
...
void Joystick::readEv()
{
	...
	if (bytes == sizeof(*joystick_ev)) {
		...
		if(buttonPressed(PUSE_BUTTON_A)){
			...
			ROS_INFO("Load Map");
			//cout<<"o/p:"<<joystick_ev->value<<endl;
			btn_id = PUSE_BUTTON_BACK;
			//btn_last = btn_id;
		}
		/*else if((int)joystick_ev->type == 2 && (int)joystick_ev->number == 5 &&buttonPressed_LRT(PUSE_BUTTON_LT)){
			cout<<"Left  btm:"<<endl;
		...
		// else if((int)joystick_ev->type == 2 && (int)joystick_ev->number == 5 && buttonPressed_LRT(PUSE_BUTTON_RT))
		// {
		//  	ROS_INFO("RT");
		// 	btn_id = PUSE_BUTTON_RT;
		// 	Throttle_Pedal = (joystick_ev->value+32767.)/(32767.*2);
		// 	cout<<"Throttle value:"<<Throttle_Pedal<<endl;
		...
		// }
		//  else if((int)joystick_ev->type == 2 && (int)joystick_ev->number == 2 && buttonPressed_LRT(PUSE_BUTTON_LT))
		//  {
		//  	ROS_INFO("LT");
		// 	btn_id = PUSE_BUTTON_LT;
		// 	Brake_Pedal = (joystick_ev->value+32767.)/(32767.*2);
		// 	cout<<"Brake value:"<<Brake_Pedal<<endl;
		//  	//std::vector<NODE_recv> NodeSet;
		// }
...
EV_Pedal_position Joystick::EV_Pedal_Position()
{

	if((int)joystick_ev->type == 2 && (int)joystick_ev->number == 5)
	{
		Throttle_Pedal = (joystick_ev->value+32767.)/(32767.*2);
		pedal_id = PUSE_BUTTON_RT;
	}
	else if((int)joystick_ev->type == 2 && (int)joystick_ev->number == 2)
	{
		Brake_Pedal = (joystick_ev->value+32767.)/(32767.*2);
		pedal_id = PUSE_BUTTON_LT;
	}
	std::cout<<"Throttle value:"<<Throttle_Pedal<<std::endl;
	std::cout<<"Brake value:"<<Brake_Pedal<<std::endl;
	std::cout <<"-------------------------------"<<std::endl;
...
bool Joystick::buttonPressed_LRT(int n) {

	return n > -1 && n < buttons+2 ? joystick_st->button[n] : 0;
	std::cout << "---------------------------------------" <<std::endl;
}
...
void Joystick::RevProcess(double receive_period)
{
	...
	while(1){
		...
		if(!(checkPause())){
			...
			//EV Pedal
			JoyStick::joystick Pedal_msg;

			Pedal_msg.btn_id =btn_id;
												float pedal_rt = Throttle_Pedal;
												float pedal_lt = Brake_Pedal;
												Pedal_msg.x = pedal_rt;
												Pedal_msg.y = pedal_lt;
			Pedal_msg.z = 0.0;

			EV_Pedal_Publisher_.publish(Pedal_msg);



			// std_msgs::Float32 Pedal_msg;
			//
			// if(pedal_id == PUSE_BUTTON_RT)
			// {
			// 	Pedal_msg.data = Throttle_Pedal;
			// 	//std::cout<<"Pedal_msg_RT: "<<Pedal_msg.data<<std::endl;
			// }
			// else if(pedal_id == PUSE_BUTTON_LT)
			// {
			// 	Pedal_msg.data = Brake_Pedal;
			// 	//std::cout<<"Pedal_msg_LT: "<<Pedal_msg.data<<std::endl;
			// }
			// EV_Pedal_Publisher_.publish(Pedal_msg);
			// std::cout<<"Pedal_msg.x: "<<Pedal_msg.x<<std::endl;
			// std::cout<<"Pedal_msg.y: "<<Pedal_msg.y<<std::endl;
			// // //std::cout<<"--------------------------------"<<std::endl;
